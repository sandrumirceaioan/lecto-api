...
export class LocationsService {
	...
	async save(location: CreateLocationDTO): Promise<Location> {
		...
		location.resort = this.sharedService.toObjectId(location.resort);
	...
	async find(query, options?): Promise<Location[]> {
		...
		return this.locationModel.find(query).sort(options.sort).skip(options.skip).limit(options.limit).select(options.select).populate('createdBy', 'email').populate('resort').lean();
	}

	async findOne(query, options?): Promise<Location> {
		options = this.sharedService.validateOptions(options);
		return this.locationModel.findOne(query).select(options.select).populate('createdBy', 'email').populate('resort').lean();
	}

	async findById(id, options?): Promise<Location> {
		options = this.sharedService.validateOptions(options);
		return this.locationModel.findById(id).select(options.select).populate('createdBy', 'email').populate('resort').lean();
	}

	async findByIds(ids: string[], options?) {
		options = this.sharedService.validateOptions(options);
		return this.locationModel.find({ _id: { $in: ids } }).sort(options.sort).skip(options.skip).limit(options.limit).select(options.select).populate('createdBy', 'email').populate('resort').lean();
	}
	...
	async findByIdAndUpdate(id, update, options?): Promise<Location> {
		update.createdBy = this.sharedService.toObjectId(update.createdBy);
		update.resort = this.sharedService.toObjectId(update.resort);
		options = this.sharedService.validateOptions(options);
		return this.locationModel
